...
void List::selectionSort(bool (*comp)(const Shape*, const Shape*))//параметр дл€ сортировки передавать
{
...
List::List(const List& other) : m_size(other.m_size) { //выдел€етс€ нова€ пам€ть, копирование
...
List& List::operator=(const List& other){//пам€ть уже есть, копирование
    if (this == &other)
    {
        return *this;
    }
...
List::List(List&& other) noexcept : m_size(other.m_size){ //выдел€етс€ нова€ пам€ть, перемещение
...
List& List::operator=(List&& other) noexcept{//пам€ть уже есть, перемещение
    if (this == &other) {
        return *this;
    }
